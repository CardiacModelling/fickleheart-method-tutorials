...
def _create_theano_likelihood_graph_voltage(data, X, ind_X, n_X, n_inducing_X,
        approx='FITC'):
    """ 
    Here we use theano to compile a comutational graph defining our discrepancy
    likelihood. Note it just compiles this graph as a C program which will
...
def _create_theano_conditional_graph_voltage(data, X, ind_X, X_new,
        approx='FITC'):
    rho = tt.dvector('rho')
    ker_sigma = tt.dscalar('ker_sigma')
    sigma = tt.dscalar('sigma')
    ...
    if approx == "FITC":
        ...
        Lamd = tt.ones_like(Qffd) * sigma2
    A_l = A / Lamd
    L_B = cholesky(tt.eye(inducing_time_V.shape[0]) \
            + tt.dot(A_l, tt.transpose(A)))
    r = y - current
    r_l = r / Lamd
    c = solve_lower(L_B, tt.dot(A, r_l))
...
class DiscrepancyLogLikelihood(pints.ProblemLogLikelihood):
    ...
    """
    def __init__(self, problem, inducing_times, voltage=None,
            inducing_voltage=None, downsample=None, temperature=None):
        ...
        if self._voltage is not None:
            ...
            self._loglikelihood = _create_theano_likelihood_graph_voltage(data,
                    x, ind_x, self._nt, self._nu)
        else:
            self._nds = 3
            self._loglikelihood = _create_theano_likelihood_graph(data, t,
                    ind_t, self._nt, self._nu)
