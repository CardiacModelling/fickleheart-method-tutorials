...
savedir = './out/mcmc-' + info_id + '-gp-tv'
...
for i in fit_idx:
    ...
    transform_x0_list.append(np.hstack((
                    transform_from_model_param(p[:-n_non_model_p]),
                    np.log(p[-n_non_model_p:]))))
...
for i, c in enumerate(chains):
    ...
    chains_param[i, :, :-n_non_model_p] = transform_to_model_param(
            c_tmp[:, :-n_non_model_p])
    # Then the discrepancy ones
    chains_param[i, :, -n_non_model_p:] = np.exp((c_tmp[:, -n_non_model_p:]))
    del(c_tmp)
...
x0 = np.append(transform_to_model_param(transform_x0[:-n_non_model_p]),
        np.exp(transform_x0[-n_non_model_p:]))
